...
def calculate_LCN(
    L: float, T_max: float,
    qp: np.ndarray, v: ndarray, dqp: np.ndarray
):
    dt = 0.001
    T = 0.
    while T <= T_max:
        symplectic_cuda(qp, dqp, L, diff_eqn_cuda, dt)
        symplectic_var_cuda(v, dqp, qp, L, var_eqn_cuda, dt)
        T += dt
...
class PlotSALI(PlotValues):
    ...
    def __init__(
        ...
        self.name = f'L={self.L}_P={self.E}_T={T}_res={resolution}'
    ...
    def plot_SALI(
        ...
        plt.pcolormesh(self.r_mesh, self.pr_mesh, self.SALI_mesh, cmap="plasma", vmin=-12, vmax=0, rasterized=True)
        plt.xlabel('$r$')
        plt.ylabel('$p_r$')
        plt.colorbar()
        if save_plot:
    ...
    def generate_LCN(self, block_size: int = 8):
        grid_size = self.resolution//block_size + 1
        cuda_qp = cuda.to_device(self.qp_mesh)
        cuda_v = cuda.to_device(self.v1_mesh)
        cuda_dqp_mesh = cuda.device_array_like(np.empty_like(self.qp_mesh))
        print(np.shape(self.v1_mesh))
        time = perf_counter()
        cuda_LCN[
            ...
            self.L, self.T,
            cuda_qp, cuda_v, cuda_dqp_mesh
        )
        self.v1_mesh = cuda_v.copy_to_host().reshape(*cuda_v.copy_to_host().shape[:-2], -1)
        print(f'time taken = {perf_counter()-time}')
        print("Calculating LCN")
        for i, row in enumerate(self.SALI_mesh):
            for j, val in enumerate(row):
                if self.qp_mesh[i, j, 1, 1] == self.qp_mesh[i, j, 1, 1]:
                    self.SALI_mesh[i, j] = np.log10(np.linalg.norm(self.v1_mesh[i, j]))/self.T
    ...
    def plot_LCN(
        ...
        if name:
            ...
            self.name = f"LCN_{self.name}"
        if load_array:
            try:
                self.SALI_mesh = self.load(self.name)
            except FileNotFoundError:
                print('file not found, generating LCN')
                self.generate_LCN(block_size)
        else:
            print('generating LCN')
            self.generate_LCN(block_size)
        ...
        self.plot_zzc(r_buff=0, pr_buff=0)
        cmap = matplotlib.cm.get_cmap("plasma", 3)
        plt.pcolormesh(self.r_mesh, self.pr_mesh, self.SALI_mesh, cmap="plasma_r", rasterized=True)
        plt.xlabel('$r$')
        plt.ylabel('$p_r$')
        plt.colorbar()
        if save_plot:
...
def SALI_value_plot():
    ...
    cb = plt.colorbar()
    cb.ax.set_yticklabels(["{:%}".format(i) for i in cb.get_ticks()])
    plt.xlabel("L")
    plt.ylabel("P")
