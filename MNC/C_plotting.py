...
class PlotTrajectory(PlotValues):
    ...
    def plot_trajectory(self, color=None, plot_zvc = True):
        self.get_trajectory()
        self.plot_zvc(self.q_points[2]*1.05)
        if self.E > 1:
            self.plot_asym()
        if not color:
            plt.plot(self.q_points[0], self.q_points[2], color=color, rasterized=True)
        else:
    ...
    def plot_3d_trajectory(self):
        ...
        ax.plot(self.q_points[0]*np.cos(self.q_points[1]), self.q_points[0]*np.sin(self.q_points[1]), self.q_points[2], rasterized=True)
        ax.set_xlabel("x")
        ax.set_ylabel("y")
        ax.set_zlabel("z")
        ax.set_xticks([0])
